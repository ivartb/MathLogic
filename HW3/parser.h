...
Node * parseLowLevelMultiplied(const string &s, int &ptr) {
	...
	if (c >= 'a' && c <= 'z') {
		...
		if (ptr < (int)s.length() && s[ptr] == '(') {
			...
			if (ptr == (int)s.length() || s[ptr++] != ')') throw ") after function end expected";
		...
		if (ptr == (int)s.length() || s[ptr++] != ')') throw ") in parseMultiplied expected";
		return res;
	}
	else if (c == '0') {
		ptr++;
...
Node * parsePredicate(const string &s, int &ptr) {
	...
	if (c >= 'A' && c <= 'Z') {
		...
		if (ptr == (int)s.length() || s[ptr++] != ')') throw ") after predicate end expected";
		return new Node(name, terms);
	}
	else {
